...
shinyServer(function(input, output, session) {
  ...
  output$WQImap <- renderLeaflet({
    #oink
    #rr <- tags$div( HTML('<img border="0" alt="Water Quality Index" src="JackPhoto.JPG" width="5" height="55"> </a>'))
  ...
  output$param3map <- renderLeaflet({
    ...
    #si se quiere añadir imagen rr <- tags$div( HTML('<img border="0" alt="Water Quality Index" src="JackPhoto.JPG" width="5" height="55"> </a>'))
    leaflet()%>% addTiles %>% 
  ...
  output$barPlot <- renderPlotly({
    # barploty <- plot_ly()
    # barploty <- bardata_percent1 %>% filter(vars == input$Param) %>% group_by(Station.Name) %>%
    ...
    # 
    #   add_bars(y=t(bardata_percent1),x=~Year) %>% 
    #   add_bars(y=t(bardata_percent1),x=~Year) %>%
    #  
    #   layout(barmode = "relative", xaxis = list(type = "multicategory"), showlegend=T, yaxis = list(title = 'Percentage (%)'))
    #      })
